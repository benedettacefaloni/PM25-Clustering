...
\end{align*}
For the so-called cohesion function $c$ either the same function as in \Cref{subsec:sPPM-model} (that is proportional to the partition probabilities in a random partition model derived from a Dirichlet process), or a uniform cohesion $c \equiv 1$ can be chosen. \medskip
...
hyperparameter combinations and cohesion functions, the non-spatially informed Lower Std version consistently outperforms thea
